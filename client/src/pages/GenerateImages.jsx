        ...
        <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-1g border border-gray-200'>
            ...
            <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                {imageStyle.map((item) => (
                    ...
                    className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border border-gray-300'} `} key={item} >{item}</span>
                ))}
            </div>
